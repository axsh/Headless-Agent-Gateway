...
4. **こまめな Git コミット**:
    * 実装計画の各ステップを完了するごとに、目的や意味のある単位で `git add` → `git commit` を実施する。
    * 大きな変更を一度にまとめてコミットすることは避ける。
    * コミットメッセージは変更内容が分かる簡潔な英語で記述する（例: `feat: add auth middleware`, `test: add validation tests`）。
...
1. **Build & Unit Test (必須)**:
    ...
    * **実行コマンド**:
        ...
        ```
    * **重要**: このステップが失敗（Exit Code != 0）した場合、**絶対に**次のステップ（統合テスト）に進んではいけない。コードが壊れている。
2. **統合テストの実施**:
...
1. **テスト失敗時の対応 (The Fix Loop)**:
    ...
    * 修正が完了したら `git add` → `git commit` で修正内容をコミットする。
2. **仕様書の更新**:
...
全てのビルドとテストが成功した後、`git push` を実施してリモートリポジトリに反映する。
