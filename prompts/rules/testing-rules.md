...
新機能の動作確認は、必ずテストコードとして実装し、`tests/` 配下に配置すること。

手動でコマンドを実行して目視で確認するだけの「E2E確認」はリグレッションテストとして機能しないため、**テストコードの代替にはならない**。

> [!CAUTION]
> **手動コマンド実行のみで完了させることの禁止**:
> 手動で `cawa-client` 等を実行して動作を確認することは、開発中の理解や探索としては有用だが、
> その確認結果は必ずテストコードに変換してリグレッションテストとして残すこと。
> 「手動で確認したから大丈夫」は許容されない。

### 13.2 E2Eテストの定義

以下の条件を全て満たすテストをE2Eテストとする:

1. 実際のサーバー (HAG) をプログラム内で起動する
2. 実際の外部CLI (Claude Code等) を呼び出す
3. APIを通じてセッション作成・メッセージ送信を行う
4. レスポンスの内容を検証する
5. 副作用 (ファイル作成、状態変更等) を検証する

### 13.3 既存インフラの活用

`tests/agentservice_e2e_test.go` に定義されたヘルパー関数を積極的に活用すること:

| ヘルパー関数 | 用途 |
|---|---|
| `startE2EServer()` | テスト用HAGサーバー起動 (動的ポート割当) |
| `createE2ESession()` | セッション作成 |
| `createE2ESessionNoModel()` | モデル未指定でのセッション作成 |
| `createE2ESessionWithModel()` | 明示的モデル指定でのセッション作成 |
| `sendE2EMessage()` | メッセージ送信 (SSEレスポンス取得) |
| `parseE2ESSEEvents()` | SSEイベントのパース |
| `getE2ESession()` | セッション状態取得 |

新しいE2Eテストを追加する際は、まずこれらのヘルパーで対応できないかを確認し、必要に応じてヘルパーを拡張すること。

### 13.4 E2Eテストが不要な場合

以下のケースではE2Eテストの追加を省略できるが、その理由を実装計画の Verification Plan に明記すること:

- 純粋な内部リファクタリング（外部から観測可能な動作に変更がない場合）
- 既存のE2Eテストで既にカバーされている機能の微修正
- 設定ファイルのみの変更（コード変更を伴わない場合）
