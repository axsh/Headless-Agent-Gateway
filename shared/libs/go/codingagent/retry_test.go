...
func TestIsNonRetryableError(t *testing.T) {
	tests := []struct {
		...
		{"failed to resolve API key from vault", true},
		{"vault_error", true},
		{"unexpected status 404 Not Found: model not found: gpt-4o", true},
		{"unexpected status 401", true},
		{"unexpected status 403", true},
		{"unexpected status 500 Internal Server Error", false},
		{"exit status 1", false},
