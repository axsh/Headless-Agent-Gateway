...
func TestConvertAnthropicRequestToResponses_BasicText(t *testing.T) {
	...
	if req.Model != "gpt-5.3-codex" {
		t.Errorf("Model = %q, want %q", req.Model, "gpt-5.3-codex")
	}
	if len(req.Input) != 1 {
...
func TestConvertResponsesResponseToAnthropic_TextOnly(t *testing.T) {
	...
	result, err := ConvertResponsesResponseToAnthropic([]byte(respBody), "gpt-5.3-codex")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	if resp.Model != "gpt-5.3-codex" {
		t.Errorf("Model = %q, want %q", resp.Model, "gpt-5.3-codex")
	}
...
func TestConvertResponsesStreamToAnthropic_TextStream(t *testing.T) {
	...
	err := ConvertResponsesStreamToAnthropic(reader, &buf, "gpt-5.3-codex")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
