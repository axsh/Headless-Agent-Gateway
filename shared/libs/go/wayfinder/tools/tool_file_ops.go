...
func newReadFile(workDir string) ToolHandler {
	return func(ctx context.Context, input map[string]any) (string, error) {
		path, _ := input["path"].(string)
		if path == "" {
			return "", fmt.Errorf("read_file: path is required")
		}
		absPath, err := wayfinder.ValidatePath(workDir, path)
		if err != nil {
...
func newWriteFile(workDir string, tracker *wayfinder.FileTracker) ToolHandler {
	return func(ctx context.Context, input map[string]any) (string, error) {
		path, _ := input["path"].(string)
...
func newListDirectory(workDir string) ToolHandler {
	return func(ctx context.Context, input map[string]any) (string, error) {
		path, _ := input["path"].(string)
		if path == "" {
			path = "."
...
func newCreateDirectory(workDir string, tracker *wayfinder.FileTracker) ToolHandler {
	return func(ctx context.Context, input map[string]any) (string, error) {
		path, _ := input["path"].(string)
		if path == "" {
...
func newEditFile(workDir string) ToolHandler {
	return func(ctx context.Context, input map[string]any) (string, error) {
		path, _ := input["path"].(string)
