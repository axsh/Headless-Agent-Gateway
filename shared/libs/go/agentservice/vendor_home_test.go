...
func TestVendorHomeDir(t *testing.T) {
	tests := []struct {
		...
		{"empty_work_claude", "", "claudecode", "/x", ""},
		{"empty_agent", "/ws", "", "/ws/.tern/s1", ""},
		{"unknown_agent", "/ws", "other", "/ws/.tern/s1", filepath.Join("/ws", ".other")},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := VendorHomeDir(tt.workDir, tt.agent, tt.sessionDir)
			if got != tt.want {
				t.Fatalf("VendorHomeDir(%q, %q, %q) = %q, want %q", tt.workDir, tt.agent, tt.sessionDir, got, tt.want)
			}
